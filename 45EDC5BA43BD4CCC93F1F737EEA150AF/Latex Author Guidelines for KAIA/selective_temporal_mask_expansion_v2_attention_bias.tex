...
\subsection{Optional Conservative-Mask Attention Bias}
Table~\ref{tab:attention_full100} evaluates the attention-bias extension on the conservative single-radius mask setting. We keep this experiment secondary because the main paper is about mask operating-point control. All rows share the same mask geometry and extra-mask ratio. Compared with Ours-Conservative, the attention-bias variants give modest BTE improvements. Occ $\lambda=2$ reduces BTE from 0.016681 to 0.016107, and Occ $\lambda=4$ lowers the residue proxy from 0.4116 to 0.3898, but the result should be read as optional source guidance rather than as part of the main mask-selection claim.
...
\hline
Method & BTE $\downarrow$ & Outside $\downarrow$ & Res. proxy $\downarrow$ \\
\hline\hline
Ours-Conservative & 0.016681 & 0.063104 & 0.4116 \\
Ours-Conservative + Occ $\lambda=2$ & 0.016107 & 0.063340 & 0.4046 \\
Ours-Conservative + Occ $\lambda=4$ & 0.016123 & 0.063793 & 0.3898 \\
Ours-Conservative + Occ$\times$Dist $\lambda=4$ & 0.016119 & 0.063134 & 0.4100 \\
\hline
\end{tabular}
}
\end{center}
\caption{Optional attention-bias refinement on the conservative mask setting. The extension is reported as a model-level check, not as the main contribution.}
\label{tab:attention_full100}
\end{table}

Table~\ref{tab:attention_wincount} reports clip-level win counts relative to Ours-Conservative. Occ$\times$Dist $\lambda=4$ improves BTE in 90/100 clips and has the highest count of BTE-improved clips with only a small residue-proxy penalty. Occ $\lambda=4$ improves the residue proxy in 79/100 clips, but improves Outside in only 11/100 clips. These counts show that the attention bias has a measurable but secondary effect, and that the preferred setting depends on whether the priority is boundary temporal stability, residue-proxy lowering, or conservative outside preservation.
...
\begin{tabular}{|l|c|c|c|c|}
\hline
Method & \shortstack{BTE\\improved} & \shortstack{Outside\\improved} & \shortstack{Res. proxy\\improved} & \shortstack{BTE improved\\+ small penalty} \\
\hline\hline
Occ $\lambda=2$ & 83/100 & 25/100 & 66/100 & 69/100 \\
Occ $\lambda=4$ & 75/100 & 11/100 & 79/100 & 68/100 \\
Occ$\times$Dist $\lambda=4$ & 90/100 & 39/100 & 55/100 & 77/100 \\
\hline
\end{tabular}
}
\end{center}
\caption{Clip-level win counts for the attention-bias extension relative to Ours-Conservative. ``Small penalty'' means $\Delta\mathrm{ResidueProxy}\leq+0.01$. Occ $\lambda=4$ gives the strongest residue-proxy count, while Occ$\times$Dist $\lambda=4$ gives the strongest BTE consistency.}
\label{tab:attention_wincount}
\end{table}
...
Table~\ref{tab:cost} summarizes the practical cost of each mask strategy. The preprocessing variants require no additional training or model modification. The attention-bias extension also requires no training, but it changes the attention logit computation by adding the reliability term. The proposed preprocessing uses only the binary mask sequence: temporal union, temporal occupancy, and boundary-near candidate filtering are computed with lightweight CPU image operations. Optical flow is used only for the boundary temporal error metric, not for mask generation. In the experiments, each mask or attention variant is inpainted separately so that the trade-off can be measured, but deployment requires only one selected operating point.
...
\begin{tabular}{|p{0.18\linewidth}|>{\centering\arraybackslash}p{0.09\linewidth}|>{\centering\arraybackslash}p{0.12\linewidth}|>{\centering\arraybackslash}p{0.10\linewidth}|p{0.25\linewidth}|}
\hline
Method & Train. & Model & Runs & Preproc. \\
\hline\hline
...
Ours two-radius & No & No & 1 & Base dilation, union, occupancy, and boundary-near filtering \\
Reliability bias & No & Yes & 1 & Occupancy map and attention-logit bias \\
\hline
\end{tabular}
\end{center}
\caption{Computational cost and practicality. Selective mask expansion is training-free and model-agnostic, while the occupancy reliability bias is a training-free model-level extension. Neither variant requires depth or optical flow at inference time.}
\label{tab:cost}
\end{table}

\subsection{Qualitative Results}
Figure~\ref{fig:cases} shows qualitative mask-expansion cases using the same naming as the main quantitative table. Each panel compares Input+mask, Boundary-only, Temporal union, Ours-Conservative, and Ours-Balanced. The driving success case shows that boundary-only can leave residue-like unchanged pixels, while temporal union changes more background content. The temporal-union failure case shows a large outside-region change caused by an overly broad union mask. The final case shows a limitation of the proposed mask expansion method: when the selected extra region is still too broad, it can also increase outside changes.
